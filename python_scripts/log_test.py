...
def run_pyqt_dashboard(shared_array, command_queue):
    ...
    class MainWindow(QMainWindow):
        def __init__(self, shared_mem, cmd_queue):
            ...
            self.setWindowTitle("AYDUO İHA Sistemleri | Analiz ve Telemetri İstasyonu")
            ...
            menu_items = ["🚁 Uçuş Göstergeleri", "📈 Sinyal Analizi", "⚙️ Konfig & Veri Kaydı"]
        ...
        def setup_page_configuration(self):
            ...
            title = QLabel("Sistem Kontrol ve Veri Analiz Merkezi")
            title.setStyleSheet("font-size: 24px; color: #ffffff; font-weight: bold; margin-bottom: 20px;")
            layout.addWidget(title)

            # --- KAYIT BÖLÜMÜ ---
            group_log = QGroupBox("CSV Veri Kaydı (Loglama)")
            form_log = QFormLayout()
            form_log.setSpacing(20)
            
            self.btn_toggle_log = QPushButton("🔴 OTOMATİK VERİ KAYDINI BAŞLAT (CSV)")
            self.btn_toggle_log.setStyleSheet("background-color: #e06c75; color: white; padding: 15px; font-size: 16px;")
            self.btn_toggle_log.clicked.connect(self.on_toggle_log_clicked)
            form_log.addRow("", self.btn_toggle_log)
            
            self.log_status_lbl = QLabel("Kayıt Durumu: Bekliyor...")
            self.log_status_lbl.setStyleSheet("color: #abb2bf;")
            form_log.addRow("Durum:", self.log_status_lbl)
            
            group_log.setLayout(form_log)
            layout.addWidget(group_log)

            # --- MPU BÖLÜMÜ ---
            group_mpu = QGroupBox("MPU6500 Sensör Hassasiyet Ayarları (SPI)")
            form_mpu = QFormLayout()
            form_mpu.setSpacing(20)
            
            self.combo_g = QComboBox()
            self.combo_g.addItem("± 2g", 0x00)
            ...
            self.combo_dps.addItem("± 250 dps", 0x00)
        ...
        def on_toggle_log_clicked(self):
            if not self.is_logging_active:
                self.is_logging_active = True
                self.shared_mem[23] = 1.0  # Loglama bayrağını aç
                self.btn_toggle_log.setText("🟢 KAYIT DEVAM EDİYOR... (DURDUR)")
                self.btn_toggle_log.setStyleSheet("background-color: #98c379; color: #282c34; padding: 15px; font-size: 16px;")
                self.log_status_lbl.setText("Arka planda test verileri kaydediliyor...")
                self.log_status_lbl.setStyleSheet("color: #98c379; font-weight: bold;")
            else:
                self.is_logging_active = False
                self.shared_mem[23] = 0.0  # Loglama bayrağını kapat
                self.btn_toggle_log.setText("🔴 OTOMATİK VERİ KAYDINI BAŞLAT (CSV)")
                self.btn_toggle_log.setStyleSheet("background-color: #e06c75; color: white; padding: 15px; font-size: 16px;")
                self.log_status_lbl.setText("Kayıt durduruldu. CSV dosyası kaydedildi.")
                self.log_status_lbl.setStyleSheet("color: #e5c07b;")

        def on_send_clicked(self):
            g_val = self.combo_g.currentData()
            dps_val = self.combo_dps.currentData()
            
            # Seçilen değerleri isim olarak loga yansıtmak için string ayıklıyoruz
            g_str = self.combo_g.currentText()
            dps_str = self.combo_dps.currentText()
            
            # Queue'ya komut gönder
            self.cmd_queue.put((g_val, dps_val, g_str, dps_str))
            
            self.status_lbl.setText(f"[ BAŞARILI ] Konfigürasyon paketi ({g_str}, {dps_str}) iletildi!")
...
# =========================================================
# 2. PROCESS: VPYTHON (3D Dronlar) ve SERİ PORT OKUMA & LOGLAMA
# =========================================================
def run_vpython_and_serial(shared_array, command_queue):
    ...
    lbl_sensor = label(pos=vector(-8, 5, 0), text='SENSÖR REFERANSI (SAF)', height=16, color=color.orange, box=False)
    ...
    active_g_str = "± 2g"
    active_dps_str = "± 250 dps"

    # Loglama Değişkenleri
    is_logging_now = False
    log_file = None
    csv_writer = None
    ...
    while True:
        ...
        while not command_queue.empty():
            g_val, dps_val, g_str, dps_str = command_queue.get()
            current_gyro_sens = GYRO_SENSITIVITY.get(dps_val, 131.0)
            active_g_str = g_str
            active_dps_str = dps_str
            try:
        ...
        # === LOGLAMA DOSYASI KONTROLÜ ===
        should_log = (shared_array[23] == 1.0)
        
        if should_log and not is_logging_now:
            # Kullanıcı KAYDET butonuna bastı, yeni dosya aç
            timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"AYDUO_Test_Log_{timestamp_str}.csv"
            log_file = open(filename, 'w', newline='', encoding='utf-8')
            csv_writer = csv.writer(log_file)
            # Başlıkları Yaz
            csv_writer.writerow([
                "Timestamp", "G_Config", "DPS_Config", 
                "Sensor_Roll(deg)", "FPGA_Roll(deg)", "Error_Roll(deg)",
                "Sensor_Pitch(deg)", "FPGA_Pitch(deg)", "Error_Pitch(deg)",
                "Sensor_Yaw(deg)", "FPGA_Yaw(deg)", "Error_Yaw(deg)"
            ])
            is_logging_now = True
            print(f"Log kaydı başladı: {filename}")
            
        elif not should_log and is_logging_now:
            # Kullanıcı KAYDI DURDUR butonuna bastı, dosyayı kapat
            log_file.close()
            is_logging_now = False
            print("Log kaydı durduruldu ve kaydedildi.")
        ...
        while len(buffer) >= PACKET_SIZE:
            if buffer[0:2] == HEADER and buffer[PACKET_SIZE-2:PACKET_SIZE] == FOOTER:
                ...
                yaw_sensor_rad += math.radians(f_gz / current_gyro_sens) * dt_gercek
                    
                if yaw_sensor_rad > math.pi:
                    yaw_sensor_rad -= 2 * math.pi
                elif yaw_sensor_rad < -math.pi:
                    yaw_sensor_rad += 2 * math.pi

                sens_r_deg = math.degrees(hud_roll)
                sens_p_deg = math.degrees(hud_pitch)
                sens_y_deg = math.degrees(yaw_sensor_rad)

                fpga_r_deg = (servo_x - 127.0) * (90.0 / 128.0)
                fpga_p_deg = (servo_y - 127.0) * (90.0 / 128.0)
                fpga_y_deg = (servo_z - 127.0) * (90.0 / 128.0)
                ...
                shared_array[18] = sens_r_deg
                shared_array[19] = sens_p_deg
                shared_array[20] = sens_y_deg
                
                # === CSV'YE SATIR YAZMA (Kayıt aktifse) ===
                if is_logging_now:
                    time_stamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    
                    err_r = abs(sens_r_deg - fpga_r_deg)
                    err_p = abs(sens_p_deg - fpga_p_deg)
                    err_y = abs(sens_y_deg - fpga_y_deg)
                    
                    csv_writer.writerow([
                        time_stamp, active_g_str, active_dps_str,
                        round(sens_r_deg, 2), round(fpga_r_deg, 2), round(err_r, 2),
                        round(sens_p_deg, 2), round(fpga_p_deg, 2), round(err_p, 2),
                        round(sens_y_deg, 2), round(fpga_y_deg, 2), round(err_y, 2)
                    ])
                    # Dosyaya anında yazıldığından emin olmak için flush atıyoruz
                    log_file.flush() 
                # ==========================================
                ...
                pitch_servo_rad = math.radians(fpga_p_deg)
                roll_servo_rad  = math.radians(fpga_r_deg)
                yaw_servo_rad   = math.radians(fpga_y_deg)
                ...
                <div style='background-color:#282c34; padding:15px; color:#abb2bf; font-family:sans-serif;'>
                    <div style='float:left; border-right: 2px solid #3e4451; padding-right: 15px; margin-right: 15px;'>
                        ...
                        <span style='font-size:14px;'>Hız: <b style='color:#61afef;'>{current_hz:.1f} Hz</b></span><br>
                        <span style='font-size:12px; color:#98c379;'>Aktif: {active_g_str} | {active_dps_str}</span>
                    </div>
                    
                    <div style='float:left;'>
                        <h3 style='color:#61afef; margin-top:0; margin-bottom:5px;'>Uçuş Telemetrisi</h3>
                        <b>ROLL:</b> {sens_r_deg:+06.1f}° | FPGA Çıkış: {servo_x} ({fpga_r_deg:.1f}°)<br>
                        <b>PITCH:</b> {sens_p_deg:+06.1f}° | FPGA Çıkış: {servo_y} ({fpga_p_deg:.1f}°)<br>
                        <b>YAW:</b> {sens_y_deg:+06.1f}° | FPGA Çıkış: {servo_z} ({fpga_y_deg:.1f}°)
                    </div>
...
if __name__ == '__main__':
    ...
    # Bellek boyutunu 24'e çıkardık (23. indis loglama durumunu tutuyor)
    shared_array = mp.Array('d', 24)
