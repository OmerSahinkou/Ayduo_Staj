...
def run_pyqt_dashboard(shared_array):
    ...
    # --- 1A. ÖZEL ÇİZİM: MOTOR KADRANLARI ---
    class MotorVisualizer(QWidget):
        ...
        def paintEvent(self, event):
            ...
            for i in range(3):
                ...
                # İbre
                angle_rad = math.radians(self.angles[i] - 90) # Yukarıyı 0 kabul et
    ...
    # --- 1B. ANA PENCERE VE PROFESYONEL TASARIM ---
    class MainWindow(QMainWindow):
        def __init__(self, shared_mem):
            ...
            self.sidebar.setCurrentRow(0) # İlk sayfayı aç

            # 30 ms'de bir yenile (33 FPS)
            self.timer = QTimer()
            self.timer.timeout.connect(self.update_from_shared_memory)
            self.timer.start(30)

        def apply_professional_theme(self):
            # Kurumsal / Modern Dark Theme (One Dark türevi)
            self.setStyleSheet("""
        ...
        def setup_page_dashboard(self):
            ...
            # Üst Kısım: VPython Bilgi Notu ve Motor Çıkışları
        ...
        def on_send_clicked(self):
            # UART gönderme işlemini VPython process'ine iletmek için paylaşılan belleğin 
            # özel bir indeksini (örn: index 0'ı geçici olarak negatif) tetikleyici olarak kullanabiliriz.
            # Ancak en temiz yol seri portun doğrudan VPython'da açılmış olmasıdır.
            # Konfigürasyon bayrağı: (Gerçek uygulamada IPC Queue kullanılır)
            print(f"[ARAYÜZ] G: {self.spin_g.value()}, DPS: {self.spin_dps.value()} gönderim isteği alındı.")
            pass # VPython tarafına IPC (Queue) ile bağlanmalı, şimdilik UI görseli hazırlandı.
        ...
        def update_from_shared_memory(self):
            ...
            # Sadece o an açık olan sayfanın grafiklerini güncelle (Performans tasarrufu)
