...
# ---------------------------------------------------------
# 1. SERİ PORT OKUMA & SİMÜLASYON İŞ PARÇACIĞI
# ---------------------------------------------------------
class SerialReader(QThread):
    ...
    def run(self):
        try:
            # Gerçek cihaza bağlanmayı dene
            ser = serial.Serial(self.port, self.baudrate, timeout=1)
            print(f"[BAŞARILI] {self.port} portu {self.baudrate} baud hızında açıldı. Gerçek veriler okunuyor...")
            ...
            # FPGA BAĞLI DEĞİLSE SİMÜLASYON MODUNA GEÇ
            print(f"[UYARI] FPGA bulunamadı ({e}). SİMÜLASYON MODU BAŞLATILIYOR...")
            t = 0.0
            while self.is_running:
                time.sleep(0.05) # Saniyede ~20 veri (50ms gecikme)
                t += 0.1
                
                # Sensör hareketini taklit eden dalgalar (İvme)
                ax = int(math.sin(t) * 8000)       
                ay = int(math.cos(t * 0.5) * 8000) 
                az = 16384                         
                
                # Jiroskop dalgaları
                gx, gy = 0, 0
                gz = int(math.sin(t * 2) * 2000)   
                
                # FPGA Servo Açıları (0-255 formatında)
                ang_x = int(128 + math.sin(t) * 40)       
                ang_y = int(128 + math.cos(t * 0.5) * 40)
                ang_z = int(128 + math.sin(t * 1.5) * 40)
                
                f_ax, f_ay, f_az = ax, ay, az
...
# ---------------------------------------------------------
# 2. FİZİK TABANLI DENGE SİMÜLASYONU WIDGET'I
# ---------------------------------------------------------
class BalanceSimulator(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.is_yaw:
            # Z Ekseni (Yaw) Pusula/Drone Görünümü
            ...
            # X ve Y Eksenleri Denge Platformu
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("3 Eksenli İHA Test & Denge Platformu (Simülasyon Destekli)")
        ...
        self.py_yaw_integrated = 0.0 

        pen_servo = pg.mkPen(color=(60, 255, 60), width=2.5) 
        pen_python = pg.mkPen(color=(255, 100, 100), width=2.0, style=Qt.DashLine) 
